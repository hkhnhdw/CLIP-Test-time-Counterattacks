...
def multiGPU_CLIP(args, model_image, model_text, model, images, text_tokens=None, prompt_token=None, dataset_name=None):
    ...
    # handle model possibly wrapped in DataParallel
    ...
    if cache_prompts is not None and os.path.exists(cache_prompts):
        ...
        if args is not None and getattr(args, 'advanced_text', None) == "wordnet_def":
            a_text_features = torch.load(cache_wordnet_def).to('cpu')
            text_features = (text_features + a_text_features) * 0.5
    else:
        # encode_text expects tokens on the same device as model
        text_features = model_core.encode_text(text_tokens.to(device))
    text_features = text_features / text_features.norm(dim=1, keepdim=True) # [n_class, d_emb]
    text_features = text_features.to(device)

    image_features = model_core.encode_image(images.to(device), prompt_token)
    image_features = image_features / image_features.norm(dim=1, keepdim=True) # [bs, d_emb]
    logits_per_image = image_features @ text_features.t() * model_core.logit_scale.exp()
    logits_per_text = text_features @ image_features.t() * model_core.logit_scale.exp()
