...
def load_val_dataset(args, val_dataset_name):
    ...
    #dataset lam mau
    elif val_dataset_name == 'minidata':
        # support a small local dataset organized as root/minidata/<class>/*.jpg
        print(f"Loading minidata from {args.root}/minidata")
        val_dataset = ImageFolder(os.path.join(args.root, 'minidata'), transform=preprocess224_interpolate)

    #truyen duong dan cua datasets vao day
    elif val_dataset_name == 'smoking':
        print(f"Loading smoking dataset from {args.root}/smoking_datasets")
        val_dataset = ImageFolder(os.path.join(args.root, 'smoking_datasets'), transform=preprocess224_interpolate)
    elif val_dataset_name == 'cars':
        print(f"Loading dataset from {args.root}/cars")
        val_dataset = ImageFolder(os.path.join(args.root, 'cars'), transform=preprocess224_interpolate)
    elif val_dataset_name == 'cottonweed':
        print(f"Loading dataset from {args.root}/cottonweed")
        val_dataset = ImageFolder(os.path.join(args.root, 'cottonweed'), transform=preprocess224_interpolate)


    #Nếu dùng map reduce thì khóa 3 đường dẫn trên kia, mở 3 đường dẫn dưới này để chương trình nhận nhiện các part là một datasets độc lập
    #elif val_dataset_name == 'smoking':
