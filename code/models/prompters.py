...
class PadPrompter(nn.Module):
    ...
    def forward(self, x):
        ...
        base = torch.zeros(1, 3, self.base_size, self.base_size, device=device)
        prompt = torch.cat([self.pad_left.to(device), base, self.pad_right.to(device)], dim=3)
        prompt = torch.cat([self.pad_up.to(device), prompt, self.pad_down.to(device)], dim=2)
...
class TokenPrompter(nn.Module):
    ...
    def forward(self, batch_size:int=1, device=None):
        # return prompt tokens expanded to batch and on correct device
        dev = device if device is not None else torch.device('cpu')
        p = self.prompt.to(dev)
        return p.expand(batch_size, -1, -1).contiguous()
...
class TokenPrompter_w_pos(nn.Module):
    ...
    def forward(self, batch_size:int=1, device=None):
        dev = device if device is not None else torch.device('cpu')
        return (self.prompt.to(dev) + self.pos_embedding.to(dev)).expand(batch_size, -1, -1).contiguous()
...
class TokenPrompter_w_pos_TransformerGEN(nn.Module):
    ...
    def forward(self, batch_size:int=1, device=None):
        dev = device if device is not None else torch.device('cpu')
        prompt = (self.prompt.to(dev) + self.pos_embedding.to(dev))
        out = self.transformer(prompt)
...
class FixedPatchPrompter(nn.Module):
    ...
    def forward(self, x):
        device = x.device if isinstance(x, torch.Tensor) else torch.device('cpu')
        prompt = torch.zeros([1, 3, self.isize, self.isize], device=device)
        prompt[:, :, :self.psize, :self.psize] = self.patch.to(device)
        prompt = prompt.expand(x.size(0), -1, -1, -1).contiguous()
        return x + prompt
...
class RandomPatchPrompter(nn.Module):
    ...
    def forward(self, x):
        ...
        device = x.device if isinstance(x, torch.Tensor) else torch.device('cpu')
        prompt = torch.zeros([1, 3, self.isize, self.isize], device=device)
        prompt[:, :, x_:x_ + self.psize, y_:y_ + self.psize] = self.patch.to(device)
        prompt = prompt.expand(x.size(0), -1, -1, -1).contiguous()
        return x + prompt
