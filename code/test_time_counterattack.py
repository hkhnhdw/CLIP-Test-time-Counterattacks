...
def validate(args, val_dataset_name, model, model_text, model_image,
    ...
    print(f"Evaluate with Attack method: {args.test_attack_type}", flush=True)
...
def main():
    ...
    # Force create file handler (basicConfig may be ignored if logger already configured)
    ...
    print(f"Logging to: {log_filename}", flush=True)
    logging.info(args)
    ...
    # load model (clip.load accepts a device string)
    device_str = "cuda" if torch.cuda.is_available() else "cpu"
    model, _ = clip.load('ViT-B/32', device_str, jit=False, prompt_len=0)
    ...
    # move model to device and wrap in DataParallel only if multiple GPUs
